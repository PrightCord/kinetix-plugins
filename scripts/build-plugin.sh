...
rustup target list --installed | grep -q wasm32-unknown-unknown   || { echo "run: rustup target add wasm32-unknown-unknown" >&2; exit 1; }

echo "==> building $PKG_NAME $VERSION ($PLUGIN_ID)"
( cd "$ROOT" && cargo build --release --target wasm32-unknown-unknown -p "$PKG_NAME" )

WASM="$ROOT/target/wasm32-unknown-unknown/release/${PKG_NAME//-/_}.wasm"
...
if [ -n "$SIGNING_KEY_FILE" ]; then
  cat "$WORK/plugin.wasm" "$WORK/plugin.toml" | openssl dgst -sha256 -binary > "$WORK/signing-digest.bin"
  openssl pkeyutl -sign -rawin -inkey "$SIGNING_KEY_FILE" -in "$WORK/signing-digest.bin" -out "$WORK/signature.ed25519"
  [ "$(wc -c < "$WORK/signature.ed25519")" -eq 64 ] || { echo "Ed25519 signature must be exactly 64 bytes" >&2; exit 1; }
fi
...
tar --sort=name --mtime='UTC 2020-01-01' --owner=0 --group=0 --numeric-owner -C "$WORK" -cf "$OUT" "${FILES[@]}" 2>/dev/null   || tar -C "$WORK" -cf "$OUT" "${FILES[@]}"
